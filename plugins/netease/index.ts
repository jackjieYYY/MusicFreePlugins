...
export function createMusicPlugin(config: PluginConfig) {
  const { currentSource, platformName, author = "欧皇大佬", version = "0.1.0", srcUrl } = config;

  function formatMusicItem(item: any) {
    return {
      id: item.id,
      title: item.name,
  ...
  async function searchMusicWithSource(query: string, page: number, source: string = "netease") {
    try {
